...
PROFILE="${1:-refresh_6m}"; LIST="${2:-scripts/production_scraper/master_list.txt}"; STALL_MIN="${STALL_MIN:-12}"
...
say "watchdog start profile=$PROFILE list=$LIST stall=${STALL_MIN}m"
while true; do
  sleep 60
  ...
  if [ -z "$pid" ]; then
    ...
    setsid nohup "$PY" -u scripts/production_scraper/run.py --profile "$PROFILE" --list "$LIST" >/dev/null 2>&1 </dev/null &
    sleep 30; continue
  fi
