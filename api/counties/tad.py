...
def _normalize_tad_row(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        # outreach (Mailer + Phone Tracking v2, 2026-06-03) — populated via
        # LEFT JOIN parcel_outreach_notes; defaults when no row exists.
...
def query_tad_parcels(polygon: list[list[float]]) -> ParcelQueryResult:
    ...
    flood_lookup = _fetch_flood_lookup_tad(
...
def _fetch_flood_lookup_tad(parcel_keys: list[str]) -> dict[str, dict[str, Any]]:
    """Batch spatial join: returns
    {parcel_key: {flood_zone, flood_zone_subtype, flood_bfe}}
    for TAD parcels whose centroid falls inside a FEMA flood polygon.
    ...
    try:
        ...
        logger.warning("TAD flood zone lookup failed: %s", exc)
        return {}
    finally:
        release_conn(conn)
