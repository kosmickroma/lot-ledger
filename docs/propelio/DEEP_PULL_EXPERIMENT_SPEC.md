...
## Verification script — local stealth probe (RUN BEFORE DEPLOY)

The novel risk in this experiment is the multi-pass cycling pattern itself —
will Propelio behave the same on calls 2/3/4/5/6 from the same client session,
under our jittered pacing? We've never done this before. A small terminal-only
test script lets us probe this WITHOUT deploying or touching the DB.

### Create: `scripts/test_dp_multipass_local.py`

```python
"""Local stealth probe for the multi-pass deep-pull strategy.

Runs all 6 passes through PropelioClient directly. No DB writes. No FastAPI.
Just imports the scraper, instantiates ONE client, cycles through the pass
sequence with proper jitter, and prints a per-pass breakdown.

Usage:
    python -m scripts.test_dp_multipass_local "123 Main St, Dallas, TX"

Exits 0 on success, 1 if any pass errored (especially 401/403/429).
"""
import logging
import random
import sys
...
# Mirror the same PASSES + constants from api/propelio/deep_pull.py
PASSES = [
    {"months": 24, "range_mi": 0.25, "label": "tightest"},
    {"months": 24, "range_mi": 0.5,  "label": "blocks"},
    {"months": 24, "range_mi": 1.0,  "label": "neighborhood"},
...
def main(address: str) -> int:
    ...
    # ONE client for the entire test — same as the runner does
    client = PropelioClient(username=PROPELIO_USERNAME, password=PROPELIO_PASSWORD)
    client.login()
    logger.info("login OK")

    # Pre-job warmup
    warmup = random.uniform(PRE_JOB_WARMUP_MIN_SECONDS, PRE_JOB_WARMUP_MAX_SECONDS)
    logger.info("warmup sleep %.1fs", warmup)
    time.sleep(warmup)

    # Pass 1: find_lead_id + add_cma
    ...
    logger.info("find_lead_id → lead_id=%s subject_lot=%s has_ck=%s", lead_id, subject_lot_sqft, bool(confirmation_key))

    envelope = client.add_cma(lead_id, confirmation_key, months=PASSES[0]["months"], range_mi=PASSES[0]["range_mi"])
    cma_id = str(envelope.get("id") or "")
    if not cma_id:
        logger.error("FAILED: could not extract cma_id from envelope. keys=%s", list(envelope.keys()))
        return 1
    logger.info("Pass 1 (add_cma): cma_id=%s, comps in envelope=%d",
                cma_id, len((envelope.get("data") or {}).get("sales") or []))

    seen = set()
    per_pass = []
    # Tabulate pass 1 comps
    pass_1_comps = (envelope.get("data") or {}).get("sales") or []
    pass_1_new = 0
    for c in pass_1_comps:
        key = _comp_address_key(c if isinstance(c, dict) else {})
        if key and key not in seen:
            seen.add(key)
            pass_1_new += 1
    per_pass.append({"pass": 1, "label": PASSES[0]["label"], "returned": len(pass_1_comps), "new": pass_1_new})

    # Saturation check after pass 1 (only fires at pass >= SATURATION_MIN_PASSES)
    ...
    for pass_num in range(2, len(PASSES) + 1):
        ...
        try:
            response = client.search_cma(lead_id, cma_id, months=cfg["months"], range_mi=cfg["range_mi"])
        except Exception as exc:
        ...
        logger.info("Pass %d done: returned=%d new=%d total_unique=%d", pass_num, len(comps), new_count, total_unique)
...
source .venv/bin/activate  # or however the env activates
python -m scripts.test_dp_multipass_local "123 Real Address, Dallas, TX"
```

**Run against 2-3 addresses (urban, suburban, rural) BEFORE deploying anything.**

What success looks like:
- All passes complete without exception
- Each pass returns comps (broader radius = more comps)
- `new` counts taper toward saturation by pass 4-5
- No 401/403/429 in logs

What failure looks like:
- Any pass raises an exception with HTTP 401/403/429 in the message → STOP. Investigate before proceeding. We may have already drawn attention.
- Cma_id empty after add_cma → response shape changed; verify against scraper.py:1033
- Unique total = pass 1 count exactly → distance expansion isn't surfacing new comps; strategy is wrong

This script is THE most valuable safety net we have. Run it once for each of the
3 test addresses, confirm zero errors, then deploy to preview with confidence.
